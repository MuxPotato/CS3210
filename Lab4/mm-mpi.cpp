...
void master()
{
	...
	long long time1 = wall_clock_time();
	fprintf(stderr, " --- MASTER %d (on %s): time taken to distribute=%6.2f seconds\n", myid, hostname, (time1 - before) / 1000000000.0);

	// Gather results from workers
	master_receive_result(result);

	fprintf(stderr, " --- MASTER %d (on %s): time taken to receive after distributed=%6.2f seconds\n", myid, hostname, (after - time1) / 1000000000.0);
